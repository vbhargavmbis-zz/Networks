...
b) In Phase 1 of the project, there are 3 entities: Major, Captain 1 and Captain 2. The Major behaves as the Server and the 2 Captains act as Clients. The 2 Captains open their respective input files which contain: 1)Resources and 2)Confidence and send this data to the Major through a socket using the UDP protocol. 
   The Major receives this data and displays the data sent by the respective Captain.

c) Major:Creates a socket and binds it to port number 6187(3300+2887). Gets its IP address and waits for reception of data. The first two receptions are from Captain 1 and the others from Captain 2. The data from each captain is parsed and stored in appropriate variables and displayed.
